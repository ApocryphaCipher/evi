...
class Catalog:
    ...
    def add_claim(self, statement: str, status: str, test: str | None, reference: str | None) -> int:
        stamp = now()
        cur = self.db.execute(
            "INSERT INTO claims (statement, status, test, reference, created_at, updated_at)"
            " VALUES (?, ?, ?, ?, ?, ?)",
            (statement, status, test, reference, stamp, stamp),
        )
        return cur.lastrowid

    def link(self, claim_id: int, item_id: int, role: str, detail: str | None) -> None:
        self.db.execute(
            "INSERT OR REPLACE INTO claim_evidence (claim_id, item_id, role, detail) VALUES (?, ?, ?, ?)",
            (claim_id, item_id, role, detail),
        )
